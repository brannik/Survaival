...
[RequireComponent(typeof(GatherableSpawnerControls))]
public class GatherableSpawner : NetworkBehaviour
{
    [Range(5f, 100f)]
    public float spawnTimer = 10f;
    [SerializeField] public List<Transform> spawnLocations;
    [SerializeField] private GameObject gatherableObject;
    ...
    void Start()
    {
        if (IsServer)
        {
            InvokeRepeating(nameof(PerformOnlineSpawn), 0, spawnTimer);
        }
    }
    ...
    private void PerformOnlineSpawn()
    {
        ...
        GameObject spawnedObj = Instantiate(gatherableObject, spawnLocation.position, Quaternion.identity);
